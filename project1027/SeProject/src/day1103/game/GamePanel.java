...
public class GamePanel extends JPanel {
	...
	public void createHero() {
		...
		hero = new Hero(img , 200, 200, 100, 65, 0, 0, this);
	}
	...
	public void createBg() {
		...
		gameBg[0] = new GameBg(img, 0, 0, WIDTH, HEIGHT, -3, 0);
		gameBg[1] = new GameBg(img, WIDTH, 0, WIDTH, HEIGHT, -3, 0);
	}
	...
	public void createEnemy() {
		...
		for(int i=0;i<20;i++) {
			double d = Math.random()*path.length;
			int ran = (int)d+1;
			Image img=ImageUtil.getIcon(this.getClass(), "res/game/e"+ran+".png", 50, 40).getImage();
			Enemy enemy = new Enemy(img, WIDTH-50, 50+(50*i), 50, 40, -1, 0);
	...
	public void createHp() {
		Image img=ImageUtil.getIcon(this.getClass(), "res/game/hp.png", 40, 40).getImage();
		for(int i=0;i<4;i++) {
			Hp hp = new Hp(img, 100+(42*i), 65 , 40, 40, 0, 0);
			hpList.add(hp); //블락 목록에 추가!!
		}
	}
	//게임의 상황 , 정보 출력 
	...
	public void showGameOver(Graphics2D g2) {
		...
		g2.drawString(sb.toString()  , 100, 400);
	}
